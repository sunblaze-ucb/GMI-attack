...
def loadnet(net):
    ...
    c_path = "./result_model/Context_G_set1.tar"
    i_path = "./result_model/identity_G_set1.tar"
...
if __name__ == "__main__":
    ...
    result_img_dir = "./result/inv_img_ganset"
    result_model_dir = "./result/inv_model_ganset"
    ...
    #load_my_state_dict(Net, torch.load('./result_model/Inversion_m1.tar')['state_dict'])
    #load_my_state_dict(DL, torch.load('./result_model/DL_m1.tar')['state_dict'])
    #load_my_state_dict(DG, torch.load('./result_model/DG_m1.tar')['state_dict'])

    #loadnet(Net)
    # vib [13, 14, 23, 53, 54]

    model_name = "IR152_set2"
    ...
    for epoch in range(epochs):
        ...
        for imgs in data_loader:
            ...
            mask = get_train_mask(img_size, bs)
            x_mask = x - x * mask + mpv * mask
            inp = torch.cat((x_mask, mask), dim=1)
            z = torch.randn(bs, z_dim).cuda()
            output = Net((inp, z))
        ...
        torch.save({'state_dict':Net.state_dict()}, result_model_dir + '/' + "Inversion_m2.tar")
        torch.save({'state_dict':DG.state_dict()}, result_model_dir + '/' + "DG_m2.tar")
        torch.save({'state_dict':DL.state_dict()}, result_model_dir + '/' + "DL_m2.tar")
